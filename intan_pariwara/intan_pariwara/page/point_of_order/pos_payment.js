...
erpnext.PointOfOrder.Payment = class {
	...
	initialize_numpad() {
		const me = this;
		this.number_pad = new erpnext.PointOfOrder.NumberPad({
			wrapper: this.$numpad,
			events: {
	...
	bind_events() {
		...
		frappe.ui.form.on("Pre Order", "contact_mobile", (frm) => {
		...
		frappe.ui.form.on("Pre Order", "coupon_code", (frm) => {
		...
		frappe.ui.form.on("Pre Order", "loyalty_amount", (frm) => {
