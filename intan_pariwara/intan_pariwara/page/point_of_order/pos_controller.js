erpnext.PointOfOrder.Controller = class {
	constructor(wrapper) {
	...
	fetch_opening_entry() {
		return frappe.call("intan_pariwara.intan_pariwara.page.point_of_order.point_of_order.check_opening_entry", {
	...
	create_opening_voucher() {
		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				{
					fieldtype: "Link",
					label: __("POE Profile"),
					options: "POE Profile",
					fieldname: "poe_profile",
					reqd: 1,
					get_query: () => poe_profile_query(),
				},
			],
			primary_action: async function ({ company, poe_profile }) {
				const method = "intan_pariwara.intan_pariwara.page.point_of_order.point_of_order.create_opening_voucher";
				const res = await frappe.call({
					method,
					args: { poe_profile, company },
					freeze: true,
				});
		...
		dialog.show();
		const poe_profile_query = () => {
			return {
				query: "intan_pariwara.intan_pariwara.doctype.poe_profile.poe_profile.poe_profile_query",
	...
	async prepare_app_defaults(data) {
		...
		this.poe_profile = data.poe_profile;
		...
		frappe.call({
			method: "intan_pariwara.intan_pariwara.page.point_of_order.point_of_order.get_poe_profile_data",
			args: { poe_profile: this.poe_profile },
			callback: (res) => {
	...
	prepare_menu() {
		...
		this.page.add_menu_item(__("Update POE Profile"), this.update_poe_profile.bind(this), false, "Shift+Ctrl+C");
	}
	...
	update_poe_profile(){
		frappe.call({
			method: "intan_pariwara.intan_pariwara.page.point_of_order.point_of_order.remove_opening_entry",
			freeze: true,
	...
	init_item_cart() {
		this.cart = new erpnext.PointOfOrder.ItemCart({
			wrapper: this.$components_wrapper,
			settings: this.settings,
			events: {
				get_frm: () => this.frm,
	...
	init_item_details() {
		this.item_details = new erpnext.PointOfOrder.ItemDetails({
			wrapper: this.$components_wrapper,
			settings: this.settings,
			events: {
				get_frm: () => this.frm,
	...
	init_payments() {
		this.payment = new erpnext.PointOfOrder.Payment({
			wrapper: this.$components_wrapper,
			events: {
				get_frm: () => this.frm || {},
				...
				submit_invoice: () => {
					this.frm.savesubmit().then((r) => {
						...
						frappe.show_alert({
							indicator: "green",
							message: __("Pre Order {0} created succesfully", [r.doc.name]),
						});
					});
				},
			},
	...
	init_recent_order_list() {
		this.recent_order_list = new erpnext.PointOfOrder.PastOrderList({
			wrapper: this.$components_wrapper,
			events: {
	...
	init_order_summary() {
		this.order_summary = new erpnext.PointOfOrder.PastOrderSummary({
			wrapper: this.$components_wrapper,
			events: {
				get_frm: () => this.frm,
	...
	async check_serial_no_availablilty(item_code, warehouse, serial_no) {
		...
		if (res.message.includes(serial_no)) {
			frappe.throw({
				title: __("Not Available"),
				message: __("Serial No: {0} has already been transacted into another Pre Order.", [
					serial_no.bold(),
				]),
			});
		}
